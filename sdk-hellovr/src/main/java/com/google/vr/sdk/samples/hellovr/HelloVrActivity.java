...
public class HelloVrActivity extends GvrActivity implements GvrView.StereoRenderer {
  ...
  int maxList = 100;
  float pathDistance = 3;

  ArrayList<PathElement> targetPositions = new ArrayList<PathElement>();
  ...
  boolean SaveMotionStream = true;
  ExternalStorageSession externalStorageSession = null;
  /**
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    //targetPositions.add(new PathElement(targetPosition, System.currentTimeMillis()));
    random = new Random();

    if (SaveMotionStream) {
      try {
        externalStorageSession = new ExternalStorageSession(getExternalFilesDir(Long.toString(System.currentTimeMillis())));
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
  ...
  public void onNewFrame(HeadTransform headTransform) {
    ...
    /////tututututu
    qheadRotation = new Quaternion();
    float[]quat = new float[4];
    //headTransform.
    headTransform.getQuaternion(quat, 0);
    qheadRotation.set(quat[0], quat[1], quat[2], quat[3]);
    targetPositions.add(new PathElement(qheadRotation, System.currentTimeMillis(),
            -pathDistance, random.nextInt(2),
            1));
            //random.nextInt(TARGET_MESH_COUNT)));
    if (targetPositions.size() >= maxList)
      targetPositions.remove(0);

    if (externalStorageSession != null) {
      long ctm = System.currentTimeMillis();
      externalStorageSession.AppendLog("q", quat, ctm);
      float[]ea = new float[3];
      headTransform.getEulerAngles(ea, 0);
      externalStorageSession.AppendLog("v", ea, ctm);
    }
    //////end tututututu
...
*/
    //////tututututut
    for (int a = 0; a < targetPositions.size(); a++) {
      Matrix.multiplyMM(modelView, 0, view, 0, targetPositions.get(a).ModelTarget, 0);
      Matrix.multiplyMM(modelViewProjection, 0, perspective, 0, modelView, 0);
      curTargetObject = targetPositions.get(a).curTargetObject;
      drawTarget(targetPositions.get(a).TextureId);
    }
    ///////end tutututut
  ...
  public void drawTarget(int TextureId) {
